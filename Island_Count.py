    ...
    def numIslands(self, grid):
            ...
            while q:
                ...
                for dir_r, dir_c in directions:
                    ...
                    if ((new_r_to_check in range(t_rows)) and # check if not going out of grid
                        ...
                        (new_r_to_check , new_c_to_check) not in visited):  # check if the adjacent is not visited already
                        visited.add((new_r_to_check , new_c_to_check)) # add to the queue
                        q.append((new_r_to_check , new_c_to_check))  # add to the visited
        ...
        for each_row in range(t_rows):
            for each_col in range(t_col):
                if (grid[each_row][each_col] == "1" and (each_row, each_col) not in visited):
